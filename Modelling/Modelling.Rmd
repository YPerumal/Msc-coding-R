...
trainRowNumbecrs <- createDataPartition(df_long$GammaGTValue, p=0.8, list=FALSE)
...
# set.seed(42)
rf_gridsearch <- train(Rented.Bike.Count ~ ., 
                       data = train_df,
                       method = 'ranger',
                       num.trees = 500,
                       verbose = T,
...
rf_final <- randomForest(Rented.Bike.Count ~ ., data = train_df,
                           ntree = 500, 
                           importance = TRUE,
                            mtry=mtry,
                            splitrule=splitrule,
...
rf_rmse <-  sqrt(mean((test_df$Rented.Bike.Count- rf_grid_pred)^2))
```
