...
```
Couple large outliers that need to be cleaned(maybe winsorised?). Probably data capture errors. Or those people are dead.

```{r}
# CotinineValue
box_and_hist(num_cols[4])
```
Dont think I cna use this one by the number of missing values. I remember Tinashe saying that I should use the SmokerStatus ind instead.
...
```
Need to take care of very large outliers. And the very small(impossible) values.
...
```
Large outliers. Seems like there may be a lot of zero values; not sure if it affects the quality of the feature.
...
```
Outliers again. Maybe people using different scales. Probably a high correlation with BMI; not as much of an issue for tree based methods. Might need to be removed anyway? Ask Stefan why tree based methods are robust to highly correlated values.
...
```
This one also seems like it might be full of zeroes and need to be dropped.
...
```
Max value  here looks like nonsense. Less missing vals than income fields above though.
...
```
Big values, but those are possible. Maybe banding this to take care of them e.g. >R5mil (maybe consider for above values too)
...
```
Lots of nulls.lots of zeroes. also not siure if it is something determined by gamma GT/med tests. But think it might be a drop.
Looks like they go in 2.5% jumps. Category field like Wang2021 did?
...
```
Seems more useful than above two, but still lots of zero vallues. Maybe an indicator column and then see how many non zeroes?
...
```
MonthlyIncome_Override2. need to check if this is a duplicate of MonthlyIncome_Override; maybe came from different tables.
...
```
Can probably group Acell Only Not sure it would be super relevant to Gamma GT
...
```
Customer had previous NTU more likely to have higher/lower gamma GT? Need to double checkj the definition of this field.
...
```
Proxy for are perhaps. could be handy. I do need to double check how many were recorded as 'Unkown' to see if this is very usable.
...
```
Hmm looks better populated than that other product field I had above. Maybe I can ditch that and keep this.

```{r}
# ProductSoldType
countplot_explore(char_cols[14])
```
Check with the notmal education fields which to use. Maybe I can just combine them, and when they dont match take this one. 
...
```
Coverage here looks weird. Need to compare with the other income fields and see what I should use for Income.
...
```
Fair chunk of NA in here, but think could be usable. NOt sure how it's handy related to GammaGT though.
