...
miss_forest_imp <- missForest(to_be_imputed,maxiter=1,ntree=2)
...
miss_forest_imp_less_cols <- missForest(to_be_imputed_less_cols,maxiter=1,ntree=2)
...
# wide_x_test <- wide_test_df %>% select(-GammaGTValue)
# wide_y_test <- wide_test_df %>% select(GammaGTValue)
```
...
```{r}
mf_train_less_cols_df <-  anti_join(tibble::rownames_to_column(miss_forest_imputed_less_cols_df),tibble::rownames_to_column(mf_test_df),by=c("rowname" = "rowname"))
```
