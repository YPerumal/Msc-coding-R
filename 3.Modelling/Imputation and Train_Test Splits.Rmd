...
system.time({ miss_forest_imp <- missForest(to_be_imputed, maxiter = 2,ntree = 5,mtry = 2)})
...
miss_forest_imp_less_cols <- missForest(to_be_imputed_less_cols)
```
...
system.time(
imp <-  mice(to_be_imputed,method='rf',seed = 42,m=2,maxit=5)
)
beep()
# 17 mins

# This works, removing all the high factor vards
system.time(
imp <-  mice(to_be_imputed %>% select(-PUBLIC_SECTOR_TYPE,-MARITAL_STATUS,-SumAssuredBand),
           seed = 42,m=2,maxit=2)
)
...
system.time(
mice_range_imp <- miceRanger(to_be_imputed %>% select(-PUBLIC_SECTOR_TYPE,-MARITAL_STATUS,-SumAssuredBand),m=2,maxiter = 2) 
)
beep()
```
