...
factor_cols_group_temp1 <-  tmp_factors %>% group_by(rowname) %>% summarise(across(where(is.factor), fn))


# Join the factor and numeric grouped by datasets back into one
grouped_imputed_less_cols_df <- num_cols_group_temp %>% left_join(factor_cols_group_temp,by=c('rowname'))
```
