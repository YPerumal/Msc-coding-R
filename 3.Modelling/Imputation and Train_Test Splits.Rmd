...
```{r}
# loop_and_name_mice <- function(test_df,imputation_object,i){
#     
# }
...
for (i in c(1:5)) {
    if(i==1){
        combine_imputed_all_df <- imputed_df_list[[i]]
    }
    else{
        combine_imputed_all_df <- rbind(combine_imputed_all_df,imputed_df_list[[i]])
    }
}

combine_imputed_all_df <- tibble::rownames_to_column(combine_imputed_all_df)
```

```{r}
# group by for the combined dataset 

combine_imputed_all_df %>% filter(rowname=='7e2bd1c3-5a24-4737-b174-cccabcaec20a') group_by(rowname) %>% summarise(mean(everything()))
```
