...
# }
# 
# test %>% group_by(rowname) %>% summarise(c=across(as.factor,Mode()))
...
test %>%
group_by(rowname, SmokerStatus) %>%
tally() %>%
arrange(rowname, desc(n)) %>%
summarize(freq = first(SmokerStatus))


i==1
for (x in names(test)[-1]) {
    tmp <- test[,c('rowname',x)]
    output <- tmp %>%
    group_by(rowname, paste0(x)) %>%
    tally() %>%
    arrange(rowname, desc(n)) %>%
    summarize(freq = first(x))
    print(output)
}
```
