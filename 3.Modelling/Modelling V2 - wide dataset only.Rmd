...
load("C:/Users/X464371/OneDrive - University of Cape Town/Yevashan Perumal MSc - OneDrive/Data/imput_splits_image.Rdata/modelling_image.Rdata")
```

# Basic Baseline
```{r}
std.dev <- sqrt(var(df_wide$GammaGTValue))
```
...
```{r}
rf_wide_test_rmse<- random_forest_func(wide_train_df,mf_test_df)
rf_wide_test_rmse
```


```{r}
rf_mf_all_test_rmse <- random_forest_func(mf_train_df,mf_test_df)
rf_mf_all_test_rmse
```

```{r}
rf_mf_less_cols_test_rmse <- random_forest_func(mf_train_less_cols_df,mf_test_df)
