...
random_forest_train_func <- function(train_data,test_data){
...
wide dataset
```{r}
rf_wide_test_model<- random_forest_train_func(wide_train_df %>% slice_sample(n=1000))
rf_wide_test_model_result <- test_func(rf_wide_test_model,mf_test_df)
```

miss forest dataset all
```{r}
rf_mf_all_test_rmse <- random_forest_func(mf_train_df  %>% slice_sample(n=1000),mf_test_df)
rf_mf_all_model_result <- test_func(rf_mf_all_test_rmse,mf_test_df)
```
...
loop_func_random_forest <- function(list_of_dfs,test_df){
    ...
    results_list <- list()
        ...
        results_list[i] <- random_forest_test_func(temp_model,mf_test_df)[1]
        }
...
test_func(xgb_wide_test_rmse,mf_test_df)
```
...
loop_func_xgb <- function(list_of_dfs,test_df){
    ...
    results_list <- list()
        ...
        for (i in c(1:length(list_of_dfs))) {
        # train the model
        temp_model <- xgb_func(data.frame(list_of_dfs[[i]]) %>%  slice_sample(n=100))
        # make prediction on test set and store y_pred vector
        results_list[i] <- random_forest_test_func(temp_model,mf_test_df)[1]
        }
    # put together all result vectors in dataframe
