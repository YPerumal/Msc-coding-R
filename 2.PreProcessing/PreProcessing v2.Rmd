...
```{r}
# create detect outlier function
detect_outlier_ignore_nulls <- function(x) {
...
# create remove outlier function
remove_outlier_ignore_nulls <- function(dataframe,
                            columns=names(dataframe)) {
    ...
    for (col in columns) {
        ...
        # remove observation if it satisfies outlier function
        dataframe <- dataframe[!detect_outlier_ignore_nulls(dataframe[[col]]), ]
    }
...
```{r}
data_cleaning_num_outliers_ignore_nulls <- function(data){
...
# Find all numeric columns 
    ...
    # Outliers
    data <- remove_outlier_ignore_nulls(data,columns=names(num_cols%>%select(-AGE,-ALL_BEN_COUNT,-ALL_CONTRACT_COUNT,-ANNUAL_INVESTMENT,-AUM)))
...
# t2 <- data_cleaning_nulls(t1)
t3 <- data_cleaning_num_outliers_ignore_nulls(t1) #changed here
t4 <- data_cleaning_factor_cols(t3)

t4
```

Something wrong here. Ending up with less rows than I do for the wide and long datasets
